...
Avatar.prototype.registerEvents = function() {
    ...
    this.socket.on('xmpp.avatar.subscribe', function(data, callback) {
        if (!data.of)
            return self._clientError('Missing \'of\' key', data, callback)
        self.subscribe({ to: data.of, node: self.NS_META }, callback)
    })
    this.socket.on('xmpp.avatar.unsubscribe', function(data, callback) {
        if (!data.of)
            return self._clientError('Missing \'of\' key', data, callback)
        self.unsubscribe({ to: data.of, node: self.NS_META }, callback)
    })
}
